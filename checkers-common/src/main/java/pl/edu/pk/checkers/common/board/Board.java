...
public class Board {
    ...
    public List<CheckerMove> getAvailableMoves(Position startPosition) {
        ...
        if (checkerType == CheckerType.WHITE || checkerType == CheckerType.BLACK) { // Simple pieces
        ...
        } else if (checkerType == CheckerType.WHITE_KING || checkerType == CheckerType.BLACK_KING) { // King pieces
    ...
    private void findSimpleCaptures(Position currentPosition, CheckerType checkerType, List<Position> capturedPieces, List<CheckerMove> availableMoves) {
        ...
        for (int[] direction : DIRECTIONS) {
            ...
            if (isValidPosition(landingPosition)) {
                ...
                if (isEnemy(checkerType, enemyCheckerType) && landingCheckerType == CheckerType.NONE && !capturedPieces.contains(enemyPosition)) {
                    foundFurtherJump = true;
                    List<Position> newCapturedPieces = new ArrayList<>(capturedPieces);
                    newCapturedPieces.add(enemyPosition);
                    findSimpleCaptures(landingPosition, checkerType, newCapturedPieces, availableMoves);
                }
            }
        }

        if (!foundFurtherJump && !capturedPieces.isEmpty()) {
            CheckerMove checkerMove = new CheckerMove(currentPosition, capturedPieces);
    ...
    public boolean hasMandatoryCaptures(boolean isWhitePlayer) {
        for (int row = 0; row < SIZE; row++) {
            for (int col = 0; col < SIZE; col++) {
                ...
                if (checkerType != null && checkerType != CheckerType.NONE) {
                    if ((isWhitePlayer && (checkerType == CheckerType.WHITE || checkerType == CheckerType.WHITE_KING)) ||
                            (!isWhitePlayer && (checkerType == CheckerType.BLACK || checkerType == CheckerType.BLACK_KING))) {
